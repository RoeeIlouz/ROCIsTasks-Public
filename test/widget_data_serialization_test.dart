...
class WidgetDataGenerators {
  ...
  static Task generateTask() {
    ...
    final taskId = 'task_${_random.nextInt(100000)}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  static Category generateCategory() {
    ...
    final categoryId = 'category_${_random.nextInt(100000)}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  static Event generateEvent() {
    ...
    final eventId = 'event_${_random.nextInt(100000)}_${DateTime.now().millisecondsSinceEpoch}';
...
class WidgetDataSerializer {
  ...
  static Map<String, dynamic> serializeTaskForWidget(Task task, Category? category) {
    return {
      'id': task.id,
      'title': task.title,
  ...
  static Map<String, dynamic> serializeEventForWidget(Event event) {
    final timeStr = event.allDay == true
        ? 'All Day'
        : event.start != null ? _formatTimeForDisplay(event.start!) : '';
    ...
    return {
      ...
      'dateDisplay': event.start != null ? _formatDateForDisplay(event.start!) : '',
      'category_color': '#4285F4', // Default event color
    };
  }
  
  /// Serialize task data for calendar widget (standardized format)
  static Map<String, dynamic> serializeTaskForCalendarWidget(Task task, Category? category) {
    return {
      'type': 'task',
      'id': task.id,
      'title': task.title,
      'start': task.dueDate?.toIso8601String() ?? '',
      'startDisplay': task.dueDate != null ? _formatTimeForDisplay(task.dueDate!) : '',
      'dateDisplay': task.dueDate != null ? _formatDateForDisplay(task.dueDate!) : '',
      'category_color': category != null
          ? '#${category.colorValue.toRadixString(16).padLeft(8, '0')}'
          : '',
    };
  ...
  static List<Map<String, dynamic>> mergeCalendarData(
    ...
    for (var task in tasks) {
      if (task.dueDate != null && !task.isCompleted && !(task.isDeleted ?? false)) {
  ...
  static List<Map<String, dynamic>> createScheduleData(
    ...
    for (var task in tasks) {
      if (task.dueDate != null && !task.isCompleted && !(task.isDeleted ?? false)) {
        final category = categories.where((c) => c.id == task.categoryId).firstOrNull;
        scheduleItems.add(serializeScheduleItem(
          type: 'task',
          id: task.id,
          title: task.title,
          date: task.dueDate!,
...
void main() {
  ...
  group('Widget Data Serialization Property Tests', () {
    ...
    test('Property 1: Pending Task Filtering - Feature: android-widget-data-fix, Property 1: Pending Task Filtering', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        for (final task in pendingTasks) {
          ...
          expect(task.isDeleted ?? false, isFalse,
              reason: 'Pending tasks should not be deleted');
        }
        
        // Verify that no completed or deleted tasks are included
        final completedTasks = allTasks.where((t) => t.isCompleted).toList();
        final deletedTasks = allTasks.where((t) => t.isDeleted ?? false).toList();
        
        for (final completedTask in completedTasks) {
          expect(pendingTasks, isNot(contains(completedTask)),
              reason: 'Completed tasks should not be in pending tasks list');
        }
        
        for (final deletedTask in deletedTasks) {
          expect(pendingTasks, isNot(contains(deletedTask)),
              reason: 'Deleted tasks should not be in pending tasks list');
        }
        
        // Verify that all non-completed, non-deleted tasks are included
        final expectedPendingTasks = allTasks.where((t) => 
            !t.isCompleted && !(t.isDeleted ?? false)).toList();
        expect(pendingTasks.length, equals(expectedPendingTasks.length),
            reason: 'All pending tasks should be included');
      }
    });
    
    test('Property 2: Task Title Preservation - Feature: android-widget-data-fix, Property 2: Task Title Preservation', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        final serializedData = WidgetDataSerializer.serializeTaskForWidget(task, null);
        
        // Property: For any task with a non-empty title, 
        // the widget data should contain the exact same title string
        expect(serializedData['title'], equals(task.title),
            reason: 'Task title should be preserved exactly in widget data');
        expect(serializedData['title'], isA<String>(),
            reason: 'Title should be a string');
        expect(serializedData['title'], isNotEmpty,
            reason: 'Title should not be empty');
      }
    });
    
    test('Property 3: Date Format Consistency - Feature: android-widget-data-fix, Property 3: Date Format Consistency', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        expect(displayDate, matches(displayRegex),
            reason: 'Display date should be in YYYY-MM-DD format');
        
        // Verify ISO format can be parsed
        expect(() => DateTime.parse(isoDate), returnsNormally,
        ...
        final expectedDisplayDate = WidgetDataSerializer._formatDateForDisplay(parsedIsoDate);
        expect(displayDate, equals(expectedDisplayDate),
            reason: 'Display date and ISO date should represent the same date');
      }
    });
    
    test('Property 4: Color Format Standardization - Feature: android-widget-data-fix, Property 4: Color Format Standardization', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        // Serialize task for widget
        final serializedData = WidgetDataSerializer.serializeTaskForWidget(task, category);
        ...
        expect(colorString, matches(hexRegex),
            reason: 'Color should be in #RRGGBBAA hex format');
        
        // Verify the color value matches the original category color
        final expectedColor = WidgetDataSerializer.formatColorForWidget(category.colorValue);
        expect(colorString, equals(expectedColor),
            reason: 'Serialized color should match the category color value');
      }
    });
    
    test('Property 5: Calendar Data Merging - Feature: android-widget-data-fix, Property 5: Calendar Data Merging', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        for (int j = 0; j < events.length; j++) {
          if (events[j].start == null) {
            ...
            final endTime = DateTime.now().add(Duration(days: j, hours: j + 1));
            events[j] = Event(
        ...
        final mergedData = WidgetDataSerializer.mergeCalendarData(events, tasks, categories);
        ...
        final eventItemsInMerged = mergedData.where((item) => item['type'] == 'event').toList();
        expect(eventItemsInMerged.length, equals(validEvents.length),
        ...
        final validTasks = tasks.where((t) => 
            t.dueDate != null && !t.isCompleted && !(t.isDeleted ?? false)).toList();
        final taskItemsInMerged = mergedData.where((item) => item['type'] == 'task').toList();
        expect(taskItemsInMerged.length, equals(validTasks.length),
        ...
        for (int j = 0; j < mergedData.length - 1; j++) {
          ...
          if (currentStart.isNotEmpty && nextStart.isNotEmpty) {
            ...
            expect(currentTime.isBefore(nextTime) || currentTime.isAtSameMomentAs(nextTime), isTrue,
        ...
        for (final item in mergedData) {
          expect(item, containsPair('type', anyOf('event', 'task')),
              reason: 'Each item should have a valid type');
          expect(item, contains('id'),
              reason: 'Each item should have an id');
          expect(item, contains('title'),
              reason: 'Each item should have a title');
          expect(item, contains('start'),
              reason: 'Each item should have a start time');
        }
      }
    });
    
    test('Property 6: Event Field Completeness - Feature: android-widget-data-fix, Property 6: Event Field Completeness', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        if (startDisplay.isNotEmpty && startDisplay != 'All Day') {
          ...
          expect(startDisplay, matches(timeRegex),
        ...
        if (dateDisplay.isNotEmpty) {
          expect(dateDisplay.length, greaterThan(0),
              reason: 'DateDisplay should not be empty when event has start time');
        }
      }
    });
    
    test('Property 9: Week Number Calculation - Feature: android-widget-data-fix, Property 9: Week Number Calculation', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        expect(weekNumber, greaterThanOrEqualTo(1),
        ...
        if (i < 10) {
          ...
          final knownDates = [
            {'date': DateTime(2023, 1, 1), 'expectedWeek': 52}, // Sunday, belongs to previous year's week
            {'date': DateTime(2023, 1, 2), 'expectedWeek': 1},  // Monday, first week of 2023
            {'date': DateTime(2023, 12, 31), 'expectedWeek': 52}, // Sunday, last week of 2023
            {'date': DateTime(2024, 1, 1), 'expectedWeek': 1},  // Monday, first week of 2024
            {'date': DateTime(2024, 12, 30), 'expectedWeek': 1}, // Monday, belongs to next year's week
          ];
          
          if (i < knownDates.length) {
            final knownDate = knownDates[i];
            final calculatedWeek = WidgetDataSerializer.calculateWeekNumber(knownDate['date'] as DateTime);
            
            // Note: Our implementation might differ slightly from strict ISO 8601
            // but should be consistent and within reasonable bounds
            expect(calculatedWeek, greaterThanOrEqualTo(1),
                reason: 'Known date ${knownDate['date']} should have valid week number');
            expect(calculatedWeek, lessThanOrEqualTo(53),
                reason: 'Known date ${knownDate['date']} should have valid week number');
          }
        }
        
        // Test consistency: same date should always return same week number
        final weekNumber2 = WidgetDataSerializer.calculateWeekNumber(testDate);
        expect(weekNumber, equals(weekNumber2),
            reason: 'Week number calculation should be consistent for same date');
        ...
        expect(weekDiff == 0 || weekDiff == 1 || weekDiff > 50, isTrue,
            reason: 'Adjacent dates should have reasonable week number differences');
      }
    });
    
    test('Property 10: Schedule Sorting - Feature: android-widget-data-fix, Property 10: Schedule Sorting', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        for (int j = 0; j < tasks.length; j++) {
          ...
          tasks[j].dueDate = DateTime.now().add(Duration(days: daysOffset, hours: hoursOffset));
          tasks[j].isCompleted = false;
          tasks[j].isDeleted = false;
        }
        ...
        for (int j = 0; j < events.length; j++) {
          ...
          final startTime = DateTime.now().add(Duration(days: daysOffset, hours: hoursOffset));
        ...
        final scheduleData = WidgetDataSerializer.createScheduleData(tasks, events, categories);
        ...
        for (int j = 0; j < scheduleData.length - 1; j++) {
          final currentDate = DateTime.parse(scheduleData[j]['date'] as String);
          final nextDate = DateTime.parse(scheduleData[j + 1]['date'] as String);
          
          expect(currentDate.isBefore(nextDate) || currentDate.isAtSameMomentAs(nextDate), isTrue,
              reason: 'Schedule items should be sorted chronologically. '
        ...
        final validTasks = tasks.where((t) => 
            t.dueDate != null && !t.isCompleted && !(t.isDeleted ?? false)).toList();
        final taskItemsInSchedule = scheduleData.where((item) => item['type'] == 'task').toList();
        expect(taskItemsInSchedule.length, equals(validTasks.length),
            reason: 'All valid tasks should be included in schedule data');
        
        // Verify all valid events are included
        final validEvents = events.where((e) => e.start != null).toList();
        final eventItemsInSchedule = scheduleData.where((item) => item['type'] == 'event').toList();
        expect(eventItemsInSchedule.length, equals(validEvents.length),
        ...
        for (final group in groupedByDateTime.values) {
          if (group.length > 1) {
            ...
            for (int k = 0; k < group.length - 1; k++) {
              ...
              expect(current, contains('type'),
                  reason: 'Items with same date/time should have type for consistent ordering');
              expect(next, contains('type'),
                  reason: 'Items with same date/time should have type for consistent ordering');
            }
          }
        }
    ...
    test('Property 11: Schedule Field Requirements - Feature: android-widget-data-fix, Property 11: Schedule Field Requirements', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        // Ensure events have start times
        for (int j = 0; j < events.length; j++) {
          final startTime = DateTime.now().add(Duration(days: j + 1, hours: j + 1));
          final endTime = startTime.add(Duration(hours: 1));
        ...
        for (final item in scheduleData) {
          ...
          expect(item['title'], isA<String>(),
              reason: 'Title should be a string');
          expect(item['dateDisplay'], isA<String>(),
              reason: 'DateDisplay should be a string');
          expect(item['timeDisplay'], isA<String>(),
          ...
          expect(item['id'], isA<String>(),
              reason: 'Id should be a string');
          
          // Verify field content validity
          expect(item['title'], isNotEmpty,
              reason: 'Title should not be empty');
          expect(item['type'], anyOf('task', 'event'),
          ...
          expect(dateDisplay, isNotEmpty,
              reason: 'DateDisplay should not be empty');
          expect(dateDisplay.length, greaterThan(3),
          ...
          expect(timeDisplay, isNotEmpty,
              reason: 'TimeDisplay should not be empty');
          
          // TimeDisplay should be either "All Day" or HH:mm format
          if (timeDisplay != 'All Day') {
            final timeRegex = RegExp(r'^\d{2}:\d{2}$');
            expect(timeDisplay, matches(timeRegex),
                reason: 'TimeDisplay should be in HH:mm format or "All Day", got: $timeDisplay');
          }
          
          // Verify consistency between date and display fields
          final parsedDate = DateTime.parse(item['date'] as String);
          final expectedDateDisplay = WidgetDataSerializer._formatDateForDisplay(parsedDate);
          expect(dateDisplay, equals(expectedDateDisplay),
              reason: 'DateDisplay should match the formatted version of the date field');
          
          // For non-all-day items, verify time consistency
          if (timeDisplay != 'All Day') {
            final expectedTimeDisplay = WidgetDataSerializer._formatTimeForDisplay(parsedDate);
            expect(timeDisplay, equals(expectedTimeDisplay),
                reason: 'TimeDisplay should match the formatted time of the date field');
          }
          
          // Verify additional fields that should be present
          expect(item, contains('description'),
              reason: 'Each scheduled item should have a description field');
          expect(item, contains('category_color'),
              reason: 'Each scheduled item should have a category_color field');
          expect(item, contains('isAllDay'),
          ...
          expect(item['category_color'], isA<String>(),
          ...
          if (categoryColor.isNotEmpty) {
            expect(categoryColor, startsWith('#'),
            ...
            expect(categoryColor, matches(hexRegex),
                reason: 'Category color should be in #RRGGBB or #RRGGBBAA hex format when not empty');
          }
          
          // Verify type-specific fields
          if (item['type'] == 'task') {
            expect(item, contains('priority'),
                reason: 'Task items should have a priority field');
            expect(item['priority'], isA<String>(),
                reason: 'Priority should be a string');
            expect(item['priority'], anyOf('low', 'medium', 'high'),
                reason: 'Priority should be a valid priority level');
          }
          
          if (item['type'] == 'event') {
            expect(item, contains('location'),
        ...
        final hasValidTasks = tasks.any((t) => 
            t.dueDate != null && !t.isCompleted && !(t.isDeleted ?? false));
        final hasValidEvents = events.any((e) => e.start != null);
        
        if (hasValidTasks) {
          final taskItems = scheduleData.where((item) => item['type'] == 'task').toList();
          expect(taskItems, isNotEmpty,
              reason: 'Schedule should include task items when valid tasks exist');
        }
        
        if (hasValidEvents) {
          final eventItems = scheduleData.where((item) => item['type'] == 'event').toList();
          expect(eventItems, isNotEmpty,
              reason: 'Schedule should include event items when valid events exist');
        }
      }
    });
    
    test('Property 12: Task Type Identification - Feature: android-widget-data-fix, Property 12: Task Type Identification', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        final tasks = WidgetDataGenerators.generateTasks(5);
        final events = WidgetDataGenerators.generateEvents(2); // Include some events for completeness
        final categories = WidgetDataGenerators.generateCategories(3);
        ...
        final validTasks = tasks.where((t) => 
            t.dueDate != null && !t.isCompleted && !(t.isDeleted ?? false)).toList();
        expect(taskItems.length, equals(validTasks.length),
        ...
        for (final taskItem in taskItems) {
          for (final eventItem in eventItems) {
            expect(taskItem['type'], isNot(equals(eventItem['type'])),
                reason: 'Task items and event items should have different type values');
          }
        }
        ...
        for (final item in scheduleData) {
          ...
          if (item['type'] == 'task') {
            ...
            expect(item['isAllDay'], equals(false),
    ...
    test('Property 15: Color Parsing Fallback - Feature: android-widget-data-fix, Property 15: Color Parsing Fallback', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        try {
          ...
          parsedColor = _extractColorSafely({'category_color': invalidColor}, 'category_color', '#TRANSPARENT');
        } catch (e) {
          threwException = true;
        }
        ...
        expect(threwException, isFalse,
            reason: 'Color parsing should not throw exception for invalid input: $invalidColor');
        
        expect(parsedColor, isNotNull,
            reason: 'Color parsing should return non-null value for invalid input: $invalidColor');
        
        expect(parsedColor, isA<String>(),
            reason: 'Color parsing should return string value for invalid input: $invalidColor');
        
        // For invalid colors, should return the fallback value
        expect(parsedColor, equals('#TRANSPARENT'),
            reason: 'Invalid color "$invalidColor" should return fallback value "#TRANSPARENT"');
        ...
        try {
          ...
          fail('Color parsing should not throw exception with custom fallback for: $invalidColor');
        }
        
        expect(parsedWithCustomFallback, equals(fallbackColor),
            reason: 'Invalid color "$invalidColor" should return custom fallback "$fallbackColor"');
      }
      ...
      for (final validColor in validColors) {
        ...
        expect(threwExceptionForValid, isFalse,
            reason: 'Color parsing should not throw exception for valid color: $validColor');
        
        expect(parsedValidColor, equals(validColor),
      ...
      try {
        missingColorResult = _extractColorSafely({}, 'category_color', '#DEFAULT');
      } catch (e) {
        fail('Color parsing should not throw exception for missing field');
      }
      
      expect(missingColorResult, equals('#DEFAULT'),
      ...
      expect(nullColorResult, equals('#NULL_FALLBACK'),
      ...
      final androidColorTestCases = [
        ...
        {'input': 'red', 'shouldSucceed': false}, // Android supports some color names, but we'll treat as invalid
      ...
      for (final testCase in androidColorTestCases) {
        ...
        try {
          ...
          fail('Color parsing should not throw exception for Android test case: $input');
        }
        ...
        if (shouldSucceed) {
          ...
          expect(result, equals('#ANDROID_FALLBACK'),
              reason: 'Invalid Android color "$input" should return fallback');
        }
      }
    });

    test('Property 14: Error Handling Graceful Degradation - Feature: android-widget-data-fix, Property 14: Error Handling Graceful Degradation', () {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        try {
          if (malformedJson.isEmpty || malformedJson == 'null' || malformedJson == 'undefined') {
              ...
              for (final item in jsonData) {
                if (item is Map<String, dynamic>) {
                  ...
                  if (item.containsKey('id') && item.containsKey('title')) {
                    ...
                    taskData['id'] = _extractStringSafely(item, 'id', 'unknown_id');
                    taskData['title'] = _extractStringSafely(item, 'title', 'Untitled Task');
                    taskData['dueDate'] = _extractStringSafely(item, 'dueDate', '');
                    taskData['category_color'] = _extractColorSafely(item, 'category_color', '');
                    taskData['priority'] = _extractStringSafely(item, 'priority', 'medium');
                    taskData['isCompleted'] = _extractBoolSafely(item, 'isCompleted', false);
        ...
        expect(parsedTasks, isA<List<Map<String, dynamic>>>(),
            reason: 'Parsing should always return a list, even for malformed input: $malformedJson');
        
        // Verify no null values in the result
        for (final task in parsedTasks) {
          expect(task, isA<Map<String, dynamic>>(),
              reason: 'Each parsed task should be a valid map');
          
          // Verify required fields are present and non-null
          if (task.isNotEmpty) {
            expect(task, contains('id'),
                reason: 'Valid parsed tasks should have id field');
            expect(task, contains('title'),
...
String _extractStringSafely(Map<String, dynamic> data, String key, String fallback) {
  try {
...
String _extractColorSafely(Map<String, dynamic> data, String key, String fallback) {
  try {
    final value = data[key];
    if (value == null) return fallback;
    if (value is String) {
      ...
      if (value.length != 7 && value.length != 9) return fallback; // #RRGGBB or #RRGGBBAA
...
bool _testColorParsingFallback() {
  try {
    ...
    for (final invalidColor in invalidColors) {
      final result = _extractColorSafely({'color': invalidColor}, 'color', '#000000');
      if (result != '#000000') {
    ...
    for (final validColor in validColors) {
      final result = _extractColorSafely({'color': validColor}, 'color', '#000000');
...
bool _testDateParsingFallback() {
  try {
    final invalidDates = ['invalid-date', '2023-13-45', '2023/01/01', 'tomorrow', ''];
    
    for (final invalidDate in invalidDates) {
      final result = _extractStringSafely({'date': invalidDate}, 'date', 'No date');
...
bool _testNullValueHandling() {
  try {
    ...
    return title == 'Default Title' && id == 'default_id' && dueDate == '' && color == '';
  } catch (e) {
    return false;
  }
}
